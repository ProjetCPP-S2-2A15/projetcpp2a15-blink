    ...
    , ui(new Ui::MainWindow)
{
    ...
    // Remplir la combo des critères
    ui->combo_tri_2->addItems({"ID", "Nom", "Prénom"});

    connect(ui->btn_tri, &QPushButton::clicked, this, &MainWindow::on_btn_tri_clicked);
    connect(ui->btnGetSuggestion, &QPushButton::clicked, this, &MainWindow::on_btnGetSuggestion_clicked);
}
...
void MainWindow::on_btn_pdf_clicked()
{
    ...
    html += "<h2 align='center'>Liste des Etablissements</h2><table border='1' cellspacing='0' cellpadding='4'>";
    html += "<tr>"
    ...
    QSqlQuery roleStats("SELECT adresse, COUNT(*) FROM etablissement GROUP BY adresse");
    html += "<h3>Statistiques par ADRESSE</h3>";
    while (roleStats.next()) {
        html += "adresse : " + roleStats.value(0).toString() + " → " + roleStats.value(1).toString() + "<br>";
    }
...
void MainWindow::afficherStatistiques()
{
    ...
    if (query.next()) {
        ...
        ui->label_stats->setText("Total etablissement : " + QString::number(total));
    } else {
    ...
    QString texte;
    while (roleStats.next()) {
        texte += "adresse : " + roleStats.value(0).toString() + " → " + roleStats.value(1).toString() + "\n";
    }
...
void MainWindow::on_ajout_clicked()
{
    ...
    if (test) {
        ...
        ui->tableView_7->setModel(En.afficher());
...
void MainWindow::on_supp_clicked()
{
    ...
    if (reply == QMessageBox::Yes) {
        ...
        if (test) {
            ui->tableView_4->setModel(En.afficher());
            QMessageBox::information(this, "Succès", "Suppression effectuée avec succès !");
            ui->IDsupp->clear();
        } else {
            QMessageBox::critical(this, "Erreur", "La suppression a échoué !");
...
{
    int ID = ui->mid->text().toInt();
    QString nom = ui->mname->text();
    QString prenom = ui->mpre->text();
    QString email = ui->memail->text();
    int telephone = ui->mtel->text().toInt();
    QString role = ui->mrole->text();
    QString matiere = ui->mstatus->text();
    ...
    if (!QRegularExpression("^\\d{8}$").match(ui->mtel->text()).hasMatch()) {
        QMessageBox::warning(this, "Téléphone invalide", "Le numéro de téléphone doit comporter exactement 8 chiffres !");
        return;
    }
...
void MainWindow::on_btn_tri_2_clicked()
{
    QString critere = ui->combo_tri_2->currentText();
    QSqlTableModel* model = new QSqlTableModel();
    model->setTable("enseignant");
...
void MainWindow::on_btn_pdf_2_clicked()
{
...
QList<Teacher> MainWindow::fetchTeachersFromDatabase() {
    QList<Teacher> teachers;
    QSqlQuery query("SELECT ID, NOM, PRENOM, MATIERE, ROLE FROM PROJETC.ENSEIGNANT");

    if (query.exec()) {
        while (query.next()) {
            Teacher teacher;
            teacher.id = query.value("ID").toInt();
            teacher.name = query.value("NOM").toString() + " " + query.value("PRENOM").toString();
            teacher.subject = query.value("MATIERE").toString();
            teacher.role = query.value("ROLE").toString();
            teacher.assignmentsCount = getAssignmentsCountForTeacher(teacher.id);  // Fetch assignments count for teacher
            teachers.append(teacher);
        }
    } else {
        qDebug() << "Error fetching teachers: " << query.lastError();
    }

    return teachers;
}

int MainWindow::getAssignmentsCountForTeacher(int teacherId) {
    QSqlQuery query;
    query.prepare("SELECT COUNT(*) FROM PROJETC.ASSIGNMENTS WHERE teacher_id = :teacher_id");
    query.bindValue(":teacher_id", teacherId);

    if (query.exec() && query.next()) {
        return query.value(0).toInt();
    } else {
        return 0;  // Default to 0 if no assignments
    }
}

// Function that handles the subject input and gives the suggestion
void MainWindow::on_btnGetSuggestion_clicked() {
    // Fetch the subject entered in the QLineEdit
    QString subject = ui->lineEditSubject->text();

    // Check if the input subject is valid
    if (subject.isEmpty()) {
        QMessageBox::warning(this, "Invalid Input", "Please enter a subject to get a suggestion.");
        return;
    }

    // Fetch the teachers from the database
    QList<Teacher> teachers = fetchTeachersFromDatabase();

    // Create an AIManager object
    AIManager ai;

    // Get the best teacher suggestion for the given subject
    Teacher suggestion = ai.suggestTeacherForSubject(subject, teachers);
    QString suggestedRole = ai.suggestRole(suggestion);

    // Display the suggestion in a message box (or update other UI elements)
    QMessageBox::information(this, "Teacher Suggestion",
                             "Best teacher for " + subject + ":\n"
                                                             "ID: " + QString::number(suggestion.id) + "\n"
